...
if monthNum > 6 or monthNum == 1:
    releaseDate = 'January ' + str(yearNum+1)
    releaseDateFr = 'Janvier ' + str(yearNum+1)
    nextReleaseDate = 'June ' + str(yearNum+1)
    nextReleaseDateFr = 'juin ' + str(yearNum+1)
else:
    releaseDate = 'June ' + str(yearNum)
    releaseDateFr = 'Juin ' + str(yearNum)
    nextReleaseDate = 'January ' + str(yearNum+1)
    nextReleaseDateFr = 'janvier ' + str(yearNum+1)
...
def prepareDf(df, minYr):
    ...
    df.drop(df[df['Countries or regions'].isin(constants.former_countries)].index, inplace=True)
    df.drop(df[df['Countries or regions'].isin(constants.former_countries_fr)].index, inplace=True)
...
def structureData(table, firstyear, lastyear, regdict, tradeAgreement, talist, structure, regs, hasOther):
    table= pd.pivot_table(table, values="VALUE", index='Countries or regions' , columns='year' )
    # Generate statistics
    table[structure['share']+str(lastyear)] = np.nan
    table[structure['growth']+str(lastyear-1)+'-'+str(lastyear)] = np.nan
    table[structure['cagr']+str(firstyear)+'-'+str(lastyear)] = np.nan
    
    # Subgroup each region and calculate regional rank
    tempList = [table.loc[regs].loc[['All countries']]]
    for reg in regdict : 
        tempDf = table.loc[reg['countries']]
        tempDf[structure['regRank']]=tempDf[lastyear].rank(method='min',ascending=False)
        tempList.append(table.loc[[reg['reg_name']]])
        if hasOther:
            other = tempDf[tempDf.index == structure['other'] + reg['reg_name']]
            tempDf = tempDf.drop(structure['other'] + reg['reg_name'])
    ...
    gr[structure['glRank']]=gr[lastyear].rank(method= 'min',ascending=False )
    gr = gr[structure['glRank']]
...
cname = pd.read_excel(constants.directory+'Country name conversion.xlsx','Country name')
...
result_iic = pd.merge(left=df_iic, right=cname,left_on = 'Countries or regions' ,right_on= 'Original name',how="inner")
result_uic = pd.merge(left=df_uic, right=cname,left_on = 'Countries or regions' ,right_on= 'Original name')
result_AMNE = pd.merge(left=df_AMNE, right=cname,left_on = 'Countries or regions' ,right_on= 'Original name')
result_CMNE = pd.merge(left=df_CMNE, right=cname,left_on = 'Countries or regions' ,right_on= 'Original name')
...
dfCountriesAndRegions_IIC.drop(dfCountriesAndRegions_IIC[dfCountriesAndRegions_IIC['Clean name'].isin(constants.former_countries)].index, inplace=True)
dfCountriesAndRegions_UIC.drop(dfCountriesAndRegions_UIC[dfCountriesAndRegions_UIC['Clean name'].isin(constants.former_countries)].index, inplace=True)
dfCountriesAndRegions_CMNE.drop(dfCountriesAndRegions_CMNE[dfCountriesAndRegions_CMNE['Clean name'].isin(constants.former_countries)].index, inplace=True)
dfCountriesAndRegions_AMNE.drop(dfCountriesAndRegions_AMNE[dfCountriesAndRegions_AMNE['Clean name'].isin(constants.former_countries)].index, inplace=True)
dfCountriesAndRegions_IIC_fr.drop(dfCountriesAndRegions_IIC_fr[dfCountriesAndRegions_IIC_fr['Nom'].isin(constants.former_countries_fr)].index, inplace=True)
dfCountriesAndRegions_UIC_fr.drop(dfCountriesAndRegions_UIC_fr[dfCountriesAndRegions_UIC_fr['Nom'].isin(constants.former_countries_fr)].index, inplace=True)

# Generate trade Agreement dictionaries excluding countries not in data
TA = copy.deepcopy(constants.taFull)
TA_UIC = copy.deepcopy(constants.taFull)
TA_fr = copy.deepcopy(constants.taFullFr)
TA_UIC_fr = copy.deepcopy(constants.taFullFr)
TA_CMNE = copy.deepcopy(constants.taFull)
TA_AMNE = copy.deepcopy(constants.taFull)
for ta in TA:
...
reg_CMNE =  makeRegDict(regions, dfCountriesAndRegions_CMNE, '_CMNE_')
